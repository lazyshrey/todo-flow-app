...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col justify-between flex-1 h-full select-none">
...
export default function Sidebar({
      ...
      <>
        ...
        {isCollapsed && (
          <button
            ...
            className="fixed left-4 top-4 h-10 w-10 rounded-full border border-white/10 bg-zinc-950 flex items-center justify-center text-zinc-400 hover:text-white cursor-pointer z-30 shadow-md transition-all pointer-events-auto"
          >
            <Menu className="h-5 w-5" />
          </button>
        )}

        {/* mobile drawer utilizing shadcn Sheet */}
        <Sheet open={!isCollapsed} onOpenChange={(open) => setIsCollapsed(!open)}>
          <SheetContent
            side="left"
            showCloseButton={false}
            className="p-4 bg-zinc-900 border-r border-white/5 w-[260px] text-zinc-100 flex flex-col justify-between z-50 overflow-hidden"
          >
            {/* Mobile close menu button in same alignment */}
            <button
              onClick={() => setIsCollapsed(true)}
              className="absolute h-8 w-8 rounded-full border border-white/10 bg-zinc-950 flex items-center justify-center text-zinc-400 hover:text-white cursor-pointer z-30 shadow-md left-4 top-4"
            >
              <Menu className="h-4 w-4" />
            </button>

            <div className="flex flex-col justify-between flex-1 mt-12 overflow-hidden">
              <SidebarContent
                isCollapsed={false}
  ...
  // Desktop collapsible sidebar utilizing Framer Motion
  return (
    <motion.div
      animate={{ width: isCollapsed ? 72 : 260 }}
      transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
      className="h-screen bg-zinc-900/15 backdrop-blur-2xl flex flex-col justify-between shrink-0 z-20 select-none relative p-4 border-r border-white/5 pointer-events-auto overflow-hidden"
    >
      {/* Toggle Collapse Button */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute h-8 w-8 rounded-full border border-white/10 bg-zinc-950 flex items-center justify-center text-zinc-400 hover:text-white cursor-pointer z-30 shadow-md transition-all pointer-events-auto -right-4 top-8"
      >
        {isCollapsed ? (
      ...
      {/* Content wrapper with width-transition */}
      <div
        className="flex flex-col justify-between flex-1 transition-all duration-300 overflow-hidden"
        style={{ width: isCollapsed ? 40 : 228 }}
      >
        <SidebarContent
